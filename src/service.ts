...
export interface IDatabaseConnectionManager {
  ...
  ping(): Promise<void>;
  transaction<T>(callback: (client: unknown) => Promise<T | undefined>): Promise<T | undefined>;
}
...
export interface IPersistenceFacade {
  ...
  acquireExclusiveLock(transaction: unknown): Promise<void>;
  releaseExclusiveLock(transaction: unknown): Promise<void>;

  // (Idempotent) Get the current version for the given namespace
  fetchCurrentRevision(client: unknown, namespace: string): Promise<IRevision | undefined>;

  // (Idempotent) Set the current version for the given namespace
  setCurrentRevision(client: unknown, namespace: string, revision: IRevision): Promise<void>;

  // Remove the namespace (and the revision from persistence)
  removeNamespace(client: unknown, namespace: string): Promise<void>;
}

export interface IDatabaseMigrationService {
  fetchCurrentVersion: (client: unknown, namespace: string) => Promise<string | undefined>;
  computeRevisions: (revisionDirectory: string) => IRevision[];
  upgrade: (client: unknown, namespace: string, revisionDirectory: string) => Promise<{ initialRevision?: IRevision, finalRevision?: IRevision }>;
  downgrade: (client: unknown, namespace: string, revisionDirectory: string) => Promise<{ initialRevision?: IRevision, finalRevision?: IRevision }>;
}

export class DatabaseMigrationService implements IDatabaseMigrationService {
  private readonly dao: IPersistenceFacade;

  private readonly logger: ILogger;

  constructor(options: { dao: IPersistenceFacade, logger?: ILogger }) {
    this.dao = options.dao;
    this.logger = options.logger ?? getLogger(DatabaseMigrationService.name);
  }

  public async fetchCurrentVersion(client: unknown, namespace: string): Promise<string | undefined> {
  ...
  public computeRevisions(revisionDirectory: string): IRevision[] {
    ...
    for (const file of files) {
      ...
      const version: string = previousVersion
        ? hashFile(file)
  ...
  ): Promise<{ initialRevision?: IRevision, finalRevision?: IRevision }>
  {
    ...
    let initialRevisionVersion = await this.fetchCurrentVersion(client, namespace);
